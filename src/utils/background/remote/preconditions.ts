...
export async function checkGithubAppInstalled(
  ...
  try {
    ...
    const url = `${'https://api.anthropic.com'}/api/oauth/organizations/${orgUUID}/code/repos/${owner}/${repo}`
    const headers = {
...
export async function checkGithubTokenSynced(): Promise<boolean> {
  ...
  try {
    ...
    const url = `${'https://api.anthropic.com'}/api/oauth/organizations/${orgUUID}/sync/github/auth`
    const headers = {
      ...getOAuthHeaders(accessToken),
      'x-organization-uuid': orgUUID,
    }
