 ...
 * @returns Array of directory paths containing .freecode/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const candidateSubdirs =
      subdir === 'skills' ? ['skill', 'skills'] : [subdir]
    for (const candidateSubdir of candidateSubdirs) {
      const claudeSubdir = join(current, '.freecode', candidateSubdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.freecode', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .freecode/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .freecode/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .freecode/<subdir> content — loading the main
