 ...
 * 3. Project memory (CLAUDE.md, .freecode/CLAUDE.md, and .freecode/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .freecode/CLAUDE.md, and all .md files in .freecode/rules/ are checked in each directory for Project memory
 *
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // checked-in files like CLAUDE.md and .freecode/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .freecode/CLAUDE.md (Project)
        const dotClaudePath = join(dir, '.freecode', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .freecode/rules/*.md files (Project)
        const rulesDir = join(dir, '.freecode', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .freecode/CLAUDE.md from the additional directory
        const dotClaudePath = join(dir, '.freecode', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .freecode/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.freecode', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .freecode/CLAUDE.md)
  ...
  // Process project unconditional .freecode/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .freecode/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .freecode/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .freecode/rules/ directories
  if (
    name.endsWith('.md') &&
