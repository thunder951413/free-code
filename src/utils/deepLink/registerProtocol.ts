...
const APP_NAME = 'Free Code URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'Free Code URL Handler.app'
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Free Code Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for Free Code
 ...
 * a cached flag in ~/.freecode.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.freecode (per-machine, not synced) rather than ~/.freecode.json (can sync).
