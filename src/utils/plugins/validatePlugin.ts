...
function detectManifestType(
  ...
  // Check if it's in .freecode-plugin directory
  if (dirName === '.freecode-plugin') {
 ...
 * '..' a user added to "climb out of .freecode-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .freecode-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .freecode-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .freecode-plugin/), not relative to marketplace.json. ' +
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Free Code ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Free Code accepts ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.freecode-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.freecode-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .freecode-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.freecode-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .freecode-plugin/marketplace.json or .freecode-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
