 ...
 *
 * The enabled/disabled state remains in .freecode/settings.json for per-repo control.
 *
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.freecode-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .freecode-plugin dir is present
          if (dirEntries.includes('.freecode-plugin')) {
