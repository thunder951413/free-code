/**
 * Marketplace manager for Free Code plugins
 *
 ...
 *     "installLocation": "/Users/me/.freecode/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.freecode/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Free Code bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 * - GitHub: Clones repo and looks for .freecode-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.freecode-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .freecode-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .freecode-plugin/)
      ...
      case 'directory': {
        // For directories, look for .freecode-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.freecode-plugin', 'marketplace.json')
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .freecode-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.freecode-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Free Code version. Run 'claude marketplace remove ${name}' and ` +
