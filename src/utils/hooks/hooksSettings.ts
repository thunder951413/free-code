...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.freecode/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.freecode/settings.json)'
    case 'projectSettings':
      return 'Project settings (.freecode/settings.json)'
    case 'localSettings':
      return 'Local settings (.freecode/settings.local.json)'
    case 'pluginHook':
