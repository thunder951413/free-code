...
/**
 * Resolve Free Code-specific path patterns for sandbox-runtime.
 *
 * Free Code uses special path prefixes in permission rules:
...
/**
 * Convert Free Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where Free Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.freecode', 'settings.json'))
    denyWrite.push(resolve(cwd, '.freecode', 'settings.local.json'))
  }

  // Block writes to .freecode/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .freecode/commands and
