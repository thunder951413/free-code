 ...
 * user's ~/.freecode/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.freecode/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.freecode.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.freecode/settings.json env (gh#217). policy/flag
