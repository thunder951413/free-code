 ...
 * user-controlled files (~/.freecode/settings.json and ~/.freecode.json),
 ...
 *
 * We read from global config (~/.freecode.json) and user settings
 * (~/.freecode/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.freecode/settings.json),
