...
function modelBelongsToFamily(model: string, family: string): boolean {
  ...
  // Resolve aliases like "best" → "claude-Ds-4-6" to check family membership
 ...
 * e.g. "claude-Ds-4-5" matches "claude-Ds-4-5-20251101" but not "claude-Ds-4-50".
 */
 ...
 * Supports shorthand like "Ds-4-5" (mapped to "claude-Ds-4-5") and
 * full prefixes like "claude-Ds-4-5". Resolves input aliases before matching.
 */
function modelMatchesVersionPrefix(model: string, entry: string): boolean {
  ...
  // Try the entry as-is (e.g. "claude-Ds-4-5")
  if (prefixMatchesModel(resolvedModel, entry)) {
    return true
  }
  // Try with "claude-" prefix (e.g. "Ds-4-5" → "claude-Ds-4-5")
  if (
 ...
 * When the allowlist contains both "Ds" and "Ds-4-5", the specific entry
 * takes precedence — "Ds" alone would be a wildcard, but "Ds-4-5" narrows
...
function familyHasSpecificEntries(
  ...
  for (const entry of allowlist) {
    ...
    // e.g., "Ds-4-5" or "claude-Ds-4-5-20251101" for the "Ds" family
    // Must match at a segment boundary (followed by '-' or end) to avoid
    // false positives like "Dsplan" matching "Ds"
 ...
 *
 * Matching tiers:
 * 1. Family aliases ("Ds", "sonnet", "haiku") — wildcard for the entire family,
 *    UNLESS more specific entries for that family also exist (e.g., "Ds-4-5").
 *    In that case, the family wildcard is ignored and only the specific entries apply.
 * 2. Version prefixes ("Ds-4-5", "claude-Ds-4-5") — any build of that version
 * 3. Full model IDs ("claude-Ds-4-5-20251101") — exact match only
 */
export function isModelAllowed(model: string): boolean {
  ...
  // e.g., "Ds" in ["Ds", "Ds-4-5"] should NOT directly match,
  // because the admin intends to restrict to Ds 4.5 only.
  ...
  // e.g., ["Ds"] allows all Ds, but ["Ds", "Ds-4-5"] only allows Ds 4.5.
  ...
  // Version-prefix matching: "Ds-4-5" or "claude-Ds-4-5" matches
  // "claude-Ds-4-5-20251101" at a segment boundary
  for (const entry of normalizedAllowlist) {
    if (!isModelFamilyAlias(entry) && !isModelAlias(entry)) {
