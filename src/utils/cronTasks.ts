...
const CRON_FILE_REL = join('.freecode', 'scheduled_tasks.json')
...
/**
 * Read and parse .freecode/scheduled_tasks.json. Returns an empty task list if the file
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.freecode'), { recursive: true })
